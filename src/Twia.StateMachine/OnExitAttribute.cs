...
/// <summary>
/// Defines an action to perform when exiting a state.
/// </summary>
/// <remarks>
/// <para>
/// The <see cref="OnExitAttribute"/> must only be used on <b>methods</b> that are marked with the <see cref="StateAttribute"/> or the <see cref="InitialStateAttribute"/>.
/// </para>
/// </remarks>
...
public class OnExitAttribute : Attribute
{
    ...
    /// </summary>
    /// <param name="action">The action to execute on exiting a state. Must be valid code in the state machine's context.See <see cref="Action"/>.</param>
    public OnExitAttribute(string action)
    {
        Action = action;
    }

    /// <summary>
    /// The action to execute on exiting the state.
    /// </summary>
    /// <remarks>
    /// <para>
